...
const initGame = () => {
    initTimer(30); //calling initTimer function with passing 30s as maxTime value
...
const checkWord = () => {
 ...
 if(!userWord) return alert("Please enter a word check"); // if user did'n enter anything

 //if user word doesn't matched with the correct word
 if(userWord !== correctWord) return alert(`${userWord} is not a correct word`);
